 ...
 * modified by Roberto Agostino Vitillo <ravitillo@lbl.gov>
 *
...
(function($) {
    ...
    function SlickGrid(container,data,columns,options) {
        ...
        function init() {
            ...
            $viewport.height($container.innerHeight() - $headerScroller.outerHeight() - $secondaryHeaderScroller.outerHeight());
            ...
            $secondaryHeaderScroller.bind("contextmenu.slickgrid", handleHeaderContextMenu);
            $secondaryHeaderScroller.bind("click.slickgrid", handleHeaderClick);    
        }
        ...
        function computeMinimumWidth(){
            for(var i = 0; i < columns.length; i++){
                ...
                if(m.summary !== undefined && m.flex && !m.touched){
                    var text = getFormatter(m)(-1, i, m.summary, m, columns);
                    
                    m.width = Math.min(200, Math.max(40, String(text).length * 7 + headerColumnWidthDiff));
                }
            }
        }

        function getOverflowString(string, pxlength){
            // TODO: Adapted to monospace 10px - to be generalized if really needed...
            var nchars = 25;
            var nsub = nchars - 3;
            var half = nsub / 2;
            if(string.length <= nchars) return string;
            return string.substr(0, half) + '...' + string.substr(string.length - nsub + half + 1);
        }
            ...
            function updateColumnVisibility(column){
                ...
                if(column.expanded === true){
                    for(var i in column.children)
                        updateColumnVisibility(column.children[i], columns);
                }
            }
            
            columns = [];   
            for(var i in columnDefinition){
                if(columnDefinition[i].isTop === true){
                    updateColumnVisibility(columnDefinition[i]);
                }   
            }
        }
            ...
            for (i = 0; i < columns.length; i++) {
                ...
                var header = $("<div class='ui-state-default slick-header-column-secondary " + (m.secondaryCssClass ? m.secondaryCssClass : "") + "' />")
                    .width((m.currentWidth || m.width) - headerColumnWidthDiff)
                                        .html(getFormatter(columns[i])(-1, i, m.summary, m, columns))
                    .attr("title", m.toolTip || m.name || "")
                    .data("fieldId", m.id)
                    .appendTo($secondaryHeaders);

               if(m.children.length){
                    var box = $('<span/>', {
                        'class': m.expanded ? "slick-expand-indicator slick-expand-indicator-uexp" : "slick-expand-indicator slick-expand-indicator-exp"
                    });
                    header.prepend(box);

                    box.bind('click', (function(column){
        ...
        function createColumnHeaders() {
            ...
            for (var i = 0; i < columns.length; i++) {
                ...
                var columnCopy = $.extend({}, columns[i]);
                var m = columns[i] = $.extend(columns[i], columnDefaults, columnCopy);
                var headerWidth = (m.currentWidth || m.width) - headerColumnWidthDiff;
                
                columnsById[m.id] = i;

                var header = $("<div class='ui-state-default slick-header-column h" + i + even + "' id='" + uid + m.id + "' />")
                    .html("<span class='slick-column-name'>" + getOverflowString(m.name, headerWidth) + "</span>")
                    .width(headerWidth)
                    .attr("title", m.toolTip || m.name || "")
                    .appendTo($headers);
            }
        ...
        function setupColumnSort() {    
            $secondaryHeaders.click(function(e) {               
                if ($(e.target).hasClass("slick-resizable-handle")) {
                    return;
                }

                if (self.onSort) {
                    var $col = $(e.target).closest(".slick-header-column-secondary");
                    if (!$col.length)
                        return;
        ...
        function setupColumnReorder() {
            $secondaryHeaders.sortable({
                ...
                stop: function(e) {
                    ...
                    var reorderedIds = $secondaryHeaders.sortable("toArray");
        ...
        function destroy() {
            ...
            if (options.enableColumnReorder && $secondaryHeaders.sortable) 
                $secondaryHeaders.sortable("destroy");
        ...
        function styleColumnWidth(index,width,styleCells) {
            var title = $headers.children().eq(index).css("width", width - headerColumnWidthDiff);
            columns[index].currentWidth = width;
            columns[index].touched = true;

            title.children().first().html(getOverflowString(title.attr('title'), width - headerColumnWidthDiff))
        ...
        function setSortColumn(columnId, ascending) {
            ...
            if (columnIndex != null) {
                $secondaryHeaders.children().eq(columnIndex)
        ...
        function setSelectedRows(rows) {
            ...
            for (i=0; i<rows.length; i++) {
                ...
                if (rowsCache[row] && !selectedRowsLookup[row]) {
                    ...
                    $(rowsCache[row]).removeClass("hover");
                }
            }
        ...
        function scrollTo(y) {
            ...
            if (prevScrollTop != newScrollTop) {
                scrollDirX = (prevScrollTop + oldOffset < newScrollTop + offset) ? 1 : -1;
        ...
        function restoreViewport(){
            $viewport[0].scrollTop = prevScrollTop;
            $viewport[0].scrollLeft = prevScrollLeft;
            $headerScroller[0].scrollLeft = prevScrollLeft;
            $secondaryHeaderScroller[0].scrollLeft = prevScrollLeft;
        }
        ...
        function appendRowHtml(stringArray,row) {
            ...
            var css = "slick-row" +
                (dataLoading ? " loading" : "") +
                (selectedRowsLookup[row] ? " selected" : "");

            var viewportLeft = lastRenderedScrollLeft - viewportW;
            var viewportRight = lastRenderedScrollLeft + 2*viewportW;
            var colStringArray = [];
            ...
            for (var i=0, width = 0, cols=columns.length; i<cols; i++, width += m.width) {
                var m = columns[i];
                
                if(width + m.width < viewportLeft) continue;
                if(width > viewportRight) break;
                ...
                colStringArray.push("<div class='" + cellCss + "' style='left:" + width +"px'>");
                //colStringArray.push("<div class='" + cellCss + "'>");

                // if there is a corresponding row (if not, this is the Add New row or this data hasn't been loaded yet)
                if (d) {
                    colStringArray.push(getFormatter(m)(row, i, d[m.field], m, d));
                }

                colStringArray.push("</div>");
            }

            stringArray.push("width:" + width + "px; height:" + options.rowHeight + "px;'>");
            for(var i = 0; i < colStringArray.length; i++) stringArray.push(colStringArray[i]);
        ...
        function resizeCanvas(deferredRendering) {
            ...
            if (options.autoHeight) { // use computed height to set both canvas _and_ divMainScroller, effectively hiding scroll bars.
                ...
                $viewport.height(
                        ...
                        $secondaryHeaderScroller.outerHeight());
            }

            viewportW = $viewport.innerWidth();
            viewportH = $viewport.innerHeight();
            lastRenderedScrollLeft = - 2*viewportW;
            numVisibleRows = Math.ceil(viewportH / options.rowHeight);

            // additional length to support rotated headers overflow
            var totalWidth = 121 - ((gridDataGetLength() > numVisibleRows) ? scrollbarDimensions.width - 1 : 0);
        ...
        function render() {         
            if(Math.abs(lastRenderedScrollLeft - scrollLeft) > viewportW){
        ...
        function handleScroll() {
            ...
            var scrollDistY = Math.abs(scrollTop - prevScrollTop);
            var scrollDistX = Math.abs(scrollLeft - prevScrollLeft);
            ...
            if (!scrollDistX && ! scrollDistY) return;

            scrollDirX = prevScrollTop < scrollTop ? 1 : -1;
            scrollDirY = prevScrollLeft < scrollLeft ? 1 : -1;
            prevScrollTop = scrollTop;

            // switch virtual pages if needed
            if (scrollDistY < viewportH) {
                scrollTo(scrollTop + offset);
            }
            else {
                var oldOffset = offset;
            ...
            var diffY = Math.abs(lastRenderedScrollTop - scrollTop);

            if (scrollDistY && diffY < viewportH)
                render();
            else
        ...
        function handleHeaderContextMenu(e) {
            if (self.onHeaderContextMenu && options.editorLock.commitCurrentEdit()) {
                e.preventDefault();
                var selectedElement = $(e.target).closest(".slick-header-column-secondary", ".slick-header-columns-secondary");
